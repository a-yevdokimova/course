...
class TestAppointmentConstraints(TransactionCase):
    ...
    def setUp(self, *args, **kwargs):
        ...
        self.client_id = self.env['client'].create({'name': 'Test Client'}).id
        self.master_id = self.env['master'].create({'name': 'Test Master'}).id
    ...
    def test_check_time_range_invalid(self):
        "Тестирование некорректного временного интервала"
        start_time = datetime(2024, 4, 8, 12, 0, 0)  # Статическое значение времени начала
        end_time = datetime(2024, 4, 8, 10, 0, 0)  # Статическое значение времени окончания, некорректный интервал
        with self.assertRaises(ValidationError, msg="Should raise a ValidationError if start_time is not earlier than end_time"):
